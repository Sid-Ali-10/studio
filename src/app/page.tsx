...
export default function Home() {
  ...
  const toggleFavorite = async (listingId: string) => {
    if (!user) {
      toast({ title: "Please login", description: "You need to be logged in to save favorites." });
      return;
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Listings Board</h1>
          <p className="text-muted-foreground">Find travelers or buyers across the Algerian diaspora.</p>
        </div>
        <Tabs value={filter} className="w-full md:w-auto" onValueChange={(v) => setFilter(v as any)}>
          <TabsList className="bg-card grid grid-cols-3 h-12 p-1 rounded-xl w-full md:w-64">

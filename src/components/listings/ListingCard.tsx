...
import { Plane, ShoppingBag, MapPin, Calendar, Weight, Star, Heart, MessageSquare, Trash2, Edit, Globe, Loader2, Check } from "lucide-react";
...
export function ListingCard({ listing, isFavorited, onToggleFavorite, onDelete }: ListingCardProps) {
  ...
  return (
    <Card className="listing-card overflow-hidden border-none shadow-sm hover:shadow-md transition-all flex flex-col h-full bg-card">
          ...
          {listing.type === "traveler" ? (
            <>
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                ...
                <span className="truncate"><span className="font-medium text-foreground text-xs uppercase opacity-70">From:</span> {listing.city}</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <MapPin size={16} className="text-primary shrink-0" />
                <span className="truncate"><span className="font-medium text-foreground text-xs uppercase opacity-70">To:</span> {listing.destination}</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <Calendar size={16} className="text-primary shrink-0" />
                <span><span className="font-medium text-foreground text-xs uppercase opacity-70">Arriving:</span> {listing.date}</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <Weight size={16} className="text-primary shrink-0" />
                <span><span className="font-medium text-foreground text-xs uppercase opacity-70">Weight:</span> {listing.weight} kg</span>
              </div>
            </>
          ) : (
            <>
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                ...
                <span className="truncate"><span className="font-medium text-foreground text-xs uppercase opacity-70">Source:</span> {listing.city}</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <MapPin size={16} className="text-accent shrink-0" />
                <span className="truncate"><span className="font-medium text-foreground text-xs uppercase opacity-70">City:</span> {listing.destination}</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <Calendar size={16} className="text-accent shrink-0" />
                <span><span className="font-medium text-foreground text-xs uppercase opacity-70">Desired:</span> {listing.date}</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <ShoppingBag size={16} className="text-accent shrink-0" />
                <span><span className="font-medium text-foreground text-xs uppercase opacity-70">Budget:</span> {listing.price} DA</span>
              </div>
            </>
          )}
        </div>
      ...
      <CardFooter className="pt-2 border-t flex items-center justify-between bg-muted/20">
        <Link href={`/profile/${listing.listerId}`} className="flex items-center gap-2 hover:opacity-80 transition-opacity">
        ...
        <div className="flex items-center gap-1">
          {isOwner ? (
            <>
              ...
              {onDelete && (
                <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:bg-muted rounded-full" onClick={() => onDelete(listing.id)}>
                  <Trash2 size={16} />
                </Button>
              )}
            </>
            ...
            <Button 
              ...
              size="sm" 
              className={cn(
                "gap-2 hover:bg-muted rounded-full px-4 h-8 text-xs font-bold transition-all",
